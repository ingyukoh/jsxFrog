/*********************************************************
 *  testMSP_hybrid_direct.jsx
 *  Direct caller for MSP1_hybrid with automatic file selection
 *  Processes: MolancanoDemo/FrogCol.eps, FrogBlk.eps with 2dFold/2d.eps
 *  Logs to: log.txt with [testMSP_hybrid_direct] prefix
 *********************************************************/
#target illustrator
...
(function() {
    ...
    function log(message) {
        ...
        logFile.writeln("[" + timestamp + "] [testMSP_hybrid_direct] " + message);
    }
    
    try {
        log("===== DIRECT MSP1_HYBRID CALLER STARTED =====");
        ...
        // Set global parameters for MSP1_hybrid
        ...
        // Execute MSP1_hybrid.jsx
        var mspScript = new File(scriptFolder + "/MSP1_hybrid.jsx");
        log("Looking for MSP1_hybrid.jsx at: " + mspScript.fsName);
        log("MSP1_hybrid.jsx exists: " + mspScript.exists);
        
        if (mspScript.exists) {
            log("Executing MSP1_hybrid.jsx...");
            $.evalFile(mspScript);
            ...
            if ($.global.MSP_success) {
                ...
                // Show success alert
                var message = "MSP1_hybrid completed successfully!\n\nCreated files:";
                ...
                alert("MSP1_hybrid failed!\n\nError: " + $.global.MSP_errorMessage);
            }
            
        } else {
            throw new Error("MSP1_hybrid.jsx not found at: " + mspScript.fsName);
        }
        ...
        log("===== DIRECT MSP1_HYBRID CALLER ENDED =====\n");
        logFile.close();
    }
})();