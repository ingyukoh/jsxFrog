/*********************************************************
 *  MSP1_stroke.jsx - Masked Script with Preserved Stroke
 *  Keeps stroke as separate visible outline
 *********************************************************/
#target illustrator
...
(function () {
    ...
    function processMasking(patternPath, maskPath, patternType) {
        try {
            ...
            log("Creating clipping mask with preserved stroke...");
            ...
            // Create stroke outline BEFORE turning off stroke
            ...
            if (clipPath.stroked) {
                ...
                log("Created separate stroke outline");
            }
            
            // Now configure the clipping path
            clipPath.filled = false;
            clipPath.stroked = false;
            clipPath.clipping = true;
            log("Clipping path created");
            ...
            // Add stroke outline on top if we created one
            if (strokeOutline) {
                strokeOutline.move(targG, ElementPlacement.PLACEATBEGINNING);
                log("Added stroke outline to preserve border");
            }
    ...
    try {
        ...
        log("===== MSP1_stroke.jsx COMPLETED SUCCESSFULLY =====");
        
    } catch(err) {
        $.global.MSP_success = false;
        $.global.MSP_errorMessage = err.message;
        log("===== MSP1_stroke.jsx FAILED: " + err.message + " =====");
    } finally {
        logFile.close();
    }
})();